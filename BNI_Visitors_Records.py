...
def show_back_to_main():
    """Show back to main dashboard button"""
    col1, col2 = st.columns([4, 1])
    with col2:
        if st.button("⬅️ Back to Main", key="back_to_main_member", use_container_width=True):
...
def main():
    """Main application entry point for member app"""
    try:
        ...
        # Skip login check since we're coming from main app
        ...
        with col3:
            if st.button("🏠 Back to Main", key="20"):
                st.session_state.selected_app = None
                st.session_state.logged_in = True
                st.rerun()

if __name__ == "__main__":
    # Only run if called directly (not as module)
    import streamlit as st
